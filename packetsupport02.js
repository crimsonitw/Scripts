(function() {
    ...
    // GUI (unchanged - title bar + all fields)
    ...
    sendBtn.onclick = () => {
        ...
        villages.forEach(v => {
            ...
            DEFENSE_UNITS.forEach(u => {
                ...
                // CRITICAL: Force 0 if no available (strict for ALL units)
                if (v.availTroops[u] <= 0) {
                    amt = 0;
                }
        ...
        // === FIXED FILLING: Clear everything first, then set values, force 0 where needed ===
        villages.forEach(v => {
            const row = v.rowElement;
            if (!row) return;

            // Step 1: Clear ALL inputs to 0 (prevents game defaulting to total)
            ["spear","sword","archer","spy","heavy"].forEach(u => {
            ...
            // Step 2: Set actual values (only for units we want >0)
            Object.entries(v.packet).forEach(([u, amt]) => {
                if (amt <= 0) return; // already cleared above
                const inp = row.querySelector(`input[name^="call["][name$="[${u}]"]`);
                if (inp) {
                    inp.value = amt;
                    inp.dispatchEvent(new Event('input', {bubbles:true}));
            ...
            // Step 3: Check row ONLY if we sent something
            const cb = row.querySelector('input.troop-request-selector');
            const sentSomething = Object.values(v.packet).some(amt => amt > 0);
            if (sentSomething && cb && !cb.checked) {
                cb.checked = true;
