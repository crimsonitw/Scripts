    ...
    sendBtn.onclick = () => {
        ...
        villages.forEach(v => {
            ...
            DEFENSE_UNITS.forEach(u => {
                ...
                // Strict enforcement for ALL defense units:
                // If available <= 0 → send EXACTLY 0 (no rounding, no exceptions)
                if (v.availTroops[u] <= 0) {
                    amt = 0;
                }
        ...
        // === CRITICAL FIX: FORCE 0 ON ANY UNIT WITH ZERO AVAILABLE ===
        villages.forEach(v => {
            const row = v.rowElement;
            if (!row) return;

            const cb = row.querySelector('input.troop-request-selector');
            let shouldCheckRow = false;

            // First: set ALL defense inputs to 0 if available was <= 0
            DEFENSE_UNITS.forEach(u => {
                ...
                if (inp) {
                    ...
                    } else if (v.packet[u] > 0) {
                        inp.value = v.packet[u];
                        inp.dispatchEvent(new Event('input', {bubbles:true}));
                        inp.dispatchEvent(new Event('change', {bubbles:true}));
                        shouldCheckRow = true;
                    }
            ...
            // Scouts (separate)
            const spyInp = row.querySelector(`input[name^="call["][name$="[spy]"]`);
            if (spyInp) {
                if (v.packet.spy > 0) {
                    spyInp.value = v.packet.spy;
                    spyInp.dispatchEvent(new Event('input', {bubbles:true}));
                    spyInp.dispatchEvent(new Event('change', {bubbles:true}));
                    shouldCheckRow = true;
                } else {
                    spyInp.value = "0";
                    spyInp.dispatchEvent(new Event('input', {bubbles:true}));
            ...
            // Only check the row if we actually sent something (defense or scouts)
            if (shouldCheckRow && cb && !cb.checked) {
                cb.checked = true;
                cb.dispatchEvent(new Event('change', {bubbles:true}));
                cb.dispatchEvent(new Event('click', {bubbles:true}));
            } else if (!shouldCheckRow && cb) {
                cb.checked = false;
            }
        });
