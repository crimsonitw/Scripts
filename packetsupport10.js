(function() {
    ...
    sendBtn.onclick = () => {
        ...
        if (totalK <= 0 || maxPacketK <= 0) {
            alert("Enter valid positive numbers for total and packet size.");
            return;
        }
        ...
        document.querySelectorAll("#village_troup_list tbody tr.call-village").forEach(row => {
            ...
            if (defensePop > 100 || availSpies > 0) {
                villages.push({
                    coord,
                    availTroops,
                    defensePop,
        ...
        villages.forEach(v => {
            ...
            const totalDefenseThis = v.defensePop;
            if (totalDefenseThis > 0) {
                DEFENSE_UNITS.forEach(u => {
                    ...
                    packet[u] = Math.min(amt, v.availTroops[u]);
                    packetDefensePop += (u === "heavy" ? packet[u] * HEAVY_POP : packet[u]);
                });
            }

            // Scouts: separate, capped by max per village
            ...
            // Final safety cap on defense (rounding)
            if (packetDefensePop > MAX_PACKET_DEFENSE) {
                const ratio = MAX_PACKET_DEFENSE / packetDefensePop;
                ["spear","sword","archer","heavy"].forEach(u => {
                    packet[u] = Math.floor(packet[u] * ratio);
                });
                packetDefensePop = Object.entries(packet)
                    .filter(([u]) => u !== "spy")
                    .reduce((s,[_,a])=>s+(u==="heavy"?a*HEAVY_POP:a),0);
            }

            v.packet = packet;
            v.packetPop = packetDefensePop; // only defense pop
        });
        ...
        const resultDiv = document.getElementById('gui-result');
        resultDiv.innerHTML = resultHTML;
        resultDiv.style.display = 'block';

        UI.SuccessMessage(
            `Success! Check the GUI panel for detailed results.<br>` +
            `Defense sent: <b>${sentDefenseK}k</b> pop`,
            8000
        );
    };
})();